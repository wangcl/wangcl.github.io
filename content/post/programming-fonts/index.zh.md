...
3. 这个字体在 sublime text 中字符宽度计算有问题，导致显示错乱，使其在编辑区处于不可用状态（UI 区是可用的）
...
现代化的编辑器应用支持多个字体配置（如：Visual Studio Code），当第一个指定的字体不支持当前字符时会自动依次 fallback 到下一个指定字体，直到找到支持该字符显示的字体。在这种编辑器中，可以首先选择自己喜欢的英文字体，后边再补充中文字体作为 fallback 字体，实现中英文由不同的字体显示（显示效果最好，但一般不能实现中英文 2:1 宽度比例）。
