...
Microsoft 官方出品，Microsoft Terminal 默认字体，字重相对较粗。官方还推出了一款名为 [Cascadia Next](https://github.com/microsoft/cascadia-code/releases/tag/cascadia-next) 的 CJK 字体的预览版，截止当前已经预览 2 年了，不知道该字体是否还在继续开发。
...
3. 这个字体在 sublime text 中空白字符宽度有问题，导致显示错乱，使其在编辑区处于不可用状态（UI 区却是可用的）
...
1. 部分支持连字，常见的 ">="、"->" 都未支持
2. 中英宽度比 2:1，实现方式是保留中文字符宽度，压缩英文字符宽度
3. 字体的 metadata 信息可能不满足 windows 规范要求，导致字体英文名称(Sarasa Mono SC)不可用，只能使用中文名
...
5. 中、英文的显示效果都非常好。中文的文楷非常漂亮，比正楷少装饰，字型更饱满，比黑体更有文艺范儿，不乏味；英文也很生动

### 总结

现代化的编辑器应用支持多个字体配置（如：Visual Studio Code），当第一个指定的字体不支持当前字符时会自动依次 fallback 到下一个指定字体，直到找到支持该字符显示的字体。在这种编辑器中，可以首先选择自己喜欢的英文字体，后边再补充中文字体作为 fallback 字体，实现中英文由不同的字体显示（显示效果最好，但不一定能实现中英文 2:1 宽度比例）。
